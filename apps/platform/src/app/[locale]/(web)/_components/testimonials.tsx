import QuotesIcon from '@oe/assets/icons/quote-right';
  ...
  return (
    <>
      {[...new Array(fullStars)].map((_, i) => (
        <Star key={`star-${i}-${Math.random()}`} className="h-5 w-5 fill-yellow-400 text-yellow-400" />
      ))}
      {hasHalfStar && <StarHalf className="h-5 w-5 fill-yellow-400 text-yellow-400" />}
      {[...new Array(emptyStars)].map((_, i) => (
        <Star key={`empty-star-${i}-${Math.random()}`} className="h-5 w-5 text-yellow-400" />
      ))}
    </>
  );
};
            ...
            <div key={testimonial.id} className="relative pt-8 pb-16">
              ...
              <div className="flex h-full flex-col rounded-2xl bg-white p-6 pb-16 shadow-[0px_4px_30px_0px_#F4F5F6]">
                <div className="mb-6 flex justify-center">
                  <QuotesIcon className="h-8 w-8 text-primary md:h-12 md:w-12" />
                </div>
                <div className="flex flex-grow flex-col">
                  <p className="mcaption-regular14 mb-6 text-center">{testimonial.quote}</p>
                </div>
