...
const AddressSection = memo(() => {
  ...
  if (!selectedChain) {
    return (
      <div className="space-y-3">
        <span className="font-semibold text-[#424242] text-[16px] leading-5">{tWallet('address')}</span>
  ...
  return (
    <div className="space-y-3">
      <span className="font-semibold text-[#424242] text-[16px] leading-5">Address</span>
      <button
        type="button"
      ...
      >
        <span className="overflow-hidden text-ellipsis font-normal text-[16px] leading-5">{selectedChain.address}</span>
        <Copy />
      </button>
    </div>
  );
