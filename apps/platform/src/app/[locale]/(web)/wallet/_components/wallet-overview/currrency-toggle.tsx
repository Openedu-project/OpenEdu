...
const CurrencyToggle = () => {
  const setIsVNDCurrency = useWalletVisibilityStore(state => state.setIsVNDCurrency);
  const isVNDCurrency = useWalletVisibilityStore(state => state.isVNDCurrency);

  const handleSetUSD = useCallback(() => setIsVNDCurrency(false), [setIsVNDCurrency]);
  const handleSetVND = useCallback(() => setIsVNDCurrency(true), [setIsVNDCurrency]);
  ...
  return (
    <MoreComboboxMenu
      ...
      items={[
        {
          label: 'USD',
          icon: <Image src={usdLogo.src} width={24} height={24} alt="usd logo" />,
          onClick: handleSetUSD,
          isActive: !isVNDCurrency,
        },
        {
          label: 'VND',
          icon: <Image src={vndLogo.src} width={24} height={24} alt="vnd logo" />,
