    ...
    <>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 p-0 sm:mb-1">
        <div className="flex w-fit items-center gap-2">
          <div>{icon}</div>
          <p className="font-semibold text-[14px] leading-[125%] sm:text-[16px]">{label}</p>
        </div>
        {actionBtns && <div className="flex gap-4">{actionBtns}</div>}
      </CardHeader>
      {cardContent ?? <CardContent className="p-0 pl-10 font-bold text-[24px] sm:text-[28px]">{value}</CardContent>}
    </>
  </Card>
);
