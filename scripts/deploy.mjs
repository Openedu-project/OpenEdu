...
function deploy({ appPath, envFileName }) {
  ...
  try {
    ...
    runWithEnv(
      `npx vercel@39.1.1 pull --yes --environment=production --token=${process.env.VERCEL_TOKEN}`,
      absolutePath,
      envVars
    );
    ...
    runWithEnv(
      ['npx vercel@39.1.1 build', `--token=${process.env.VERCEL_TOKEN}`, isProd ? '--prod' : '', '--yes']
        .filter(Boolean)
        .join(' '),
      absolutePath,
      envVars
