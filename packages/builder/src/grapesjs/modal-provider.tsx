import type React from 'react';
import { createElement, memo, useEffect, useState } from 'react';
import { useEditorInstance } from './context/editor-instance';
import { useEditorOptions } from './context/editor-options';
import { noop } from './utils';
import { WrapDom } from './utils/react';

export interface ModalState {
  /**
   * Modal title
   */
  title: React.ReactElement;

  /**
   * Modal content
   */
  content: React.ReactElement;

  /**
   * Modal attributes
   */

  // biome-ignore lint/suspicious/noExplicitAny: <explanation>
  attributes: Record<string, any>;

  /**
   * Callback for closing the modal
   */
  close: () => void;
}

export interface ModalResultProps extends ModalState {
  /**
   * Indicates if the modal is open.
   */
  open: boolean;
}

export interface ModalProviderProps {
  children: (props: ModalResultProps) => React.JSX.Element;
}

export interface ModalEventProps {
  open: boolean;
  title: string | HTMLElement;
  content: string | HTMLElement;
  // biome-ignore lint/suspicious/noExplicitAny: <explanation>
  attributes: Record<string, any>;
  close: () => void;
}

export const ModalProvider = memo(({ children }: ModalProviderProps) => {
  const { editor } = useEditorInstance();
  const options = useEditorOptions();
  const [isOpen, setOpen] = useState(false);
  const [modalState, setModalState] = useState<ModalState>({
    title: <></>,
    content: <></>,
    attributes: {},
    close: noop,
  });

  useEffect(() => {
    if (!editor) {
      return;
    }
    const event = 'modal';

    const toListen = ({ open, title, content, close, attributes }: ModalEventProps) => {
      open &&
        setModalState({
          title: createElement('h1', WrapDom(title)),
          content: createElement('div', WrapDom(content)),
          attributes,
          close,
        });
      setOpen(open);
    };

    editor.on(event, toListen);

    return () => {
      editor.off(event, toListen);
    };
  }, [editor]);

  useEffect(() => options.setCustomModal(true), [options.setCustomModal]);

  return editor ? (typeof children === 'function' ? children({ open: isOpen, ...modalState }) : null) : null;
});
