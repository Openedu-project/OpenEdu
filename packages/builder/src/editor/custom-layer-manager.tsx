...
import type { LayersResultProps } from '../grapesjs';
...
const getDragTarget = (ev: React.PointerEvent) => {
  ...
  const elLayer = el?.closest('[data-layer-item]') as HTMLElement & { __cmp: Component };
...
export default function CustomLayerManager({ root }: Omit<LayersResultProps, 'Container'>) {
  ...
  const onDragMove = (ev: React.PointerEvent) => {
    ...
    const canMove = Components.canMove(cmpTarget as Component, cmpSource, cmpIndex);
  ...
  const onDragEnd = () => {
    if (canMoveRes?.result) {
      canMoveRes.source?.move(canMoveRes.target as Component, { at: canMoveRes.index });
    }
    setCanMoveRes({});
  ...
  return (
    ...
    >
      ...
      {showIndicator && (
        <div ref={indicatorRef} className={cn('absolute h-0.5 w-full bg-yellow-400')} style={indicatorStyle} />
      )}
    </div>
  );
}
