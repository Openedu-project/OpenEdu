...
const AvailHomepageSolution: SectionComponent<'homepage', 'availSolution'> = ({
  ...
  return (
    <>
      <div className="bg-accent p-4 md:space-y-8 md:p-8 lg:flex lg:gap-4 lg:p-12">
        <Image alt="banner" src={props?.banner?.url} className="rounded-lg object-contain" height={864} width={708} />
      ...
      <div className="flex flex-col items-center bg-accent/80 pb-4">
        <Image
          src={line?.src}
          alt="line"
          ...
          >
            <CarouselContent className="-ml-1 flex">
              {props?.carousels.map((c, index) => (
                <CarouselItem key={index.toString()} className="pl-1 sm:basis-1/2 md:basis-1/4 lg:basis-1/8">
                  <div className="p-1">
                    <Image
                      src={c?.url}
                      alt="image"
        ...
        <Button className="mt-4 md:mt-8">
          <Link
            ...
            className={cn('!text-primary-foreground bg-inherit hover:bg-transparent hover:no-underline')}
          >
            {props?.button?.text}
          </Link>
        </Button>
