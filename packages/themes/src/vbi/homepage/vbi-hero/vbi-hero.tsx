    ...
    >
      ...
      <div className="flex flex-col items-center gap-12 md:h-[calc(100vh-var(--header-height)-120px)] md:flex-row">
        ...
        <div className="flex h-[60vh] flex-auto flex-col justify-center space-y-6 py-2 text-foreground md:h-full lg:flex-1">
          <h1 className="!leading-[1.5] font-bold text-3xl md:text-5xl ">{t?.('title')}</h1>
          <p className="text-foreground/80 text-md md:text-lg">{t?.('titleSub')}</p>

          <Button className="w-full sm:w-fit">
            <Link href={props?.button?.link ?? '#'} className="bg-inherit text-primary-foreground hover:no-underline">
              {t?.('button.text')}
            </Link>
          </Button>
        </div>
