...
import { StatementCard, type StatementCardProps } from '../../_components/statement-card';
...
const VbiAboutUsGoal: SectionComponent<'about-us', 'vbiGoal'> = ({ className }) => {
  ...
  return (
    <div className={cn('space-y-4 p-4 md:space-y-8 md:p-8 lg:p-12 bg-muted', className)}>
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <StatementCard icon="star" title={t('vision.title')} content={t('vision.content')} />
        <StatementCard icon="heart" title={t('mission.title')} content={t('mission.content')} />
      </div>
    </div>
  );
};
