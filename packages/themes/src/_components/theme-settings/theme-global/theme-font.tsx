...
const ThemeFonts = ({
  ...
  const [fontConfig, setFontConfig] = useState<ThemeGlobalFontConfig>(
    fontData || initialThemeGlobalFonts
  );

  const updateFontVariable = (
    key: keyof ThemeGlobalFontConfig,
    value: string
  ) => {
            ...
            <div className="flex items-start justify-between">
              ...
              <Select
                ...
                onValueChange={(newValue) =>
                  updateFontVariable(
                    key as keyof ThemeGlobalFontConfig,
                    newValue
                  )
                }
              >
              ...
              <div style={{ fontFamily: value }}>
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <span className="text-muted-foreground text-sm">
                    {getFontDisplayName(
                      value.replace("var(--font-", "").replace(")", "")
                    )}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <Label className="text-muted-foreground text-xs">
                      {t("preview.numbersLabel")}
                    </Label>
                    <p className="font-mono">{t("preview.numbers")}</p>
                  </div>
                  <div className="space-y-1">
                    <Label className="text-muted-foreground text-xs">
                      {t("preview.symbolsLabel")}
                    </Label>
