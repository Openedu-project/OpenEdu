...
const ThemeSettingColors = ({
  ...
  isLoading,
  colorData,
}: ThemeSettingColorsProps) => {
  const t = useTranslations("themeUI.color");
  const [isDarkMode, setIsDarkMode] = useState(false);
  const [activeTheme, setActiveTheme] = useState<ThemeMode>("light");
  const [themeConfig, setThemeConfig] = useState<ThemeGlobalColorConfig>(
    colorData || initialThemeGlobalColors
  );
                      ...
                      <Popover>
                        ...
                        <PopoverContent className="w-auto p-0" align="start">
                          <ColorPicker
                            ...
                            onColorSelect={(color) =>
                              handleVariableChange(variable, color)
                            }
                          />
                        </PopoverContent>
                      </Popover>
