...
import type { ThemeGlobalRadiusConfig, ThemeGlobalRadiusKey } from '../../../_types';
...
const ThemeBorderRadius = ({ radiusData, isLoading, onSubmitRadius }: ThemeBorderRadiusProps) => {
  const t = useTranslations('themeUI.radius');
  const [radiusConfig, setRadiusConfig] = useState<ThemeGlobalRadiusConfig>(radiusData || initialThemeGlobalRadius);
    ...
    () => (
      <div className="flex justify-center gap-8 rounded-lg border py-4">
        {(Object.keys(SIZE_MAP) as ThemeGlobalRadiusKey[]).map(size => (
          <RadiusPreview key={size} size={size} radius={radiusConfig[size]} baseRadius={radiusConfig.default} />
        ))}
      </div>
    ),
    [radiusConfig]
  ...
  return (
    <Card className="w-full rounded-none border-none shadow-none">
      ...
      <CardContent className="space-y-6">
        {(Object.keys(initialThemeGlobalRadius) as ThemeGlobalRadiusKey[]).map(key => (
            ...
            <div className="flex items-start justify-between">
              ...
              <div className="flex w-[240px] items-center justify-center gap-4">
                ...
                <Input
                  ...
                  onChange={e => updateRadiusVariable(key, Number(e.target.value))}
                  className="font-mono"
