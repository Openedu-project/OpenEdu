...
export default function FormAffiliateCampaignModal({
  ...
  const handleFormSubmit = useCallback(
    async (value: CreateAffiliateCampaignSchemaType) => {
      const submissionValue = {
        ...value,
        start_date: value.start_date ? convertToTimeStamp(value.start_date as unknown as string) : undefined,
        end_date: value.has_end_date && value.end_date ? convertToTimeStamp(value.end_date as unknown as string) : 0,
      };
  ...
  return (
    <Modal
      open={true}
      title={isCreate ? t('createAffiliateCampaign') : t('editAffiliateCampaign')}
      onClose={onClose}
      buttons={[
        {
          type: 'button',
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-center justify-between rounded-lg border-[0.4px] border-border-base-on-canvas p-4">
                  <div className="space-y-0.5">
                    <FormLabel className="text-base">{t('hasEndDate')}</FormLabel>
                  </div>
                  <FormControl>
