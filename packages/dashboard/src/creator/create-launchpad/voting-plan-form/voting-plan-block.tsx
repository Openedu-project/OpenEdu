...
import {
  type IVotingPlanLaunchpadSchemaType,
  votingPlanLaunchpadSchema,
} from "@oe/api/schemas/launchpadSchema";
...
const VotingPlanBlock = ({ onSubmit }: IVotingPlanBlockProps) => {
  ...
  const [isFinishFundingGoal, setIsFinishFundingGoal] =
    useState<boolean>(false);
  const [defaultValues, setDefaultValues] =
    useState<IVotingPlanLaunchpadSchemaType>({
  ...
  useEffect(() => {
    ...
    setVotingPhase(
      dataLaunchpadVotingPhaseRule?.value.find((rule) => {
        return (
          rule.funding_goal_gte <= targetAmount &&
          (rule.funding_goal_lt === -1 || rule.funding_goal_lt > targetAmount)
        );
      })?.max_phases ?? 0
    );
          ...
          content={
            <div className="flex items-center justify-center">
              ...
              <TabsList asChild>
                ...
                >
                  <span className="text-base text-primary underline">
                    {tLaunchpad("noFundingGoal.fundingGoal")}
                  </span>
                </TabsTrigger>
              </TabsList>
            ...
            <div className="grid w-full gap-[20px]">
              ...
              <h1 className="font-semibold text-xl">
                ...
                <b className="font-semibold text-primary">
                  {` ${form
                    .watch()
                    .clp_voting_milestones.reduce(
                      (acc, milestone) => acc + milestone.target_section,
                      0
                    )} sections`}
                </b>
              </h1>
