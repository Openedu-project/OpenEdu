...
export default async function InviteReferralProgramList() {
  const allReferralProgramListRes = await getAllReferralProgramListService(
  ...
  const myPointProfileRes = await getMyPointProfileService(
    API_ENDPOINT.USER_ME_POINT,
    { id: data?.id ?? "" }
  );
  ...
  return (
    <main className="container mx-auto max-w-[1440px] px-4 py-6">
      <InviteReferralProgramHeader
        ...
        totalBalance={
          myPointProfileRes?.point_wallets?.[0]?.available_balance ?? "0"
        }
