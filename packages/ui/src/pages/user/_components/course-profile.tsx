...
import type { ICourseMyProfile } from "@oe/api/types/user-profile";
import { Card, CardContent } from "@oe/ui/shadcn/card";
import { Checkbox } from "@oe/ui/shadcn/checkbox";
import { HoverCard, HoverCardTrigger } from "@oe/ui/shadcn/hover-card";
import { Separator } from "@oe/ui/shadcn/separator";
import { useRouter } from "next/navigation";
import type React from "react";
import { useCallback, useState } from "react";

import type { IMutateCourse } from "@oe/api/types/course/course";
              ...
              <>
                ...
                {Number(discount_price) > 0 && (
                  <p className="mcaption-regular16 text-[#999999] line-through">
                    {currency}&nbsp;
                    {formatNumber(
                      Number(price ?? 0) + Number(discount_price ?? 0)
                    )}
                  </p>
                )}
              </>
    ...
    return (
      <Card
        ...
        className={cn(
          ...
          !isSetting &&
            "hover:border-[3px] hover:border-primary hover:shadow-shadow-5",
      ...
      >
        ...
        <p className="giant-iheading-semibold16 line-clamp-2 h-[41px] text-primary group-hover:text-primary">
          {name}
        </p>
          ...
          {org && (
            <>
              ...
              <Separator
                ...
                className="h-5 bg-bg-neutral-50"
              />
            </>
          )}
          <p className="truncate">
            {partners?.filter((p) => p?.roles?.includes("owner"))?.[0]
              ?.username ?? "Unknown"}
          </p>
        </div>
