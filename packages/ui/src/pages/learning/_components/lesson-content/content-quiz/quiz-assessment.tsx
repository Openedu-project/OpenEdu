...
const QuizAssessment = ({
  ...
  const { question, has_next_question, current_question_index, start_at } =
    data;

  return (
    <div className="flex h-full max-h-full max-w-full flex-col md:flex-row">
      <div className="relative z-10 flex w-full flex-col rounded-2xl bg-primary-20 p-4 md:w-1/2 md:p-6">
        <QuizHeader
          timeLimit={
            settings?.time_limit_type === "per_question"
              ? question.settings.time_limit
              : settings?.time_limit
          }
          startAt={
            settings?.time_limit_type === "per_question"
        ...
        <Progress
          className="mt-3 mb-4 h-1"
          value={(current_question_index / numQuestion) * 100}
        />
        ...
        <div className="flex w-full flex-1 flex-col items-center justify-center">
          <div className="w-full rounded-2xl bg-white p-4 text-center shadow-shadow-6 md:p-6">
            ...
            <p className="mcaption-semibold16 text-current">
              {question?.title}
            </p>
          </div>
