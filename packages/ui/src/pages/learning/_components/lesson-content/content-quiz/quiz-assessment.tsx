...
const QuizAssessment = ({ numQuestion, data, settings, onTimeUp, quizStartAt, onSubmitAnswer }: IQuestionProps) => {
  ...
  return (
    <div className="flex h-full max-h-full max-w-full flex-col md:flex-row">
      <div className="relative z-10 flex w-full flex-col rounded-2xl bg-muted p-4 md:w-1/2 md:p-6">
        <QuizHeader
        ...
        <div className="flex w-full flex-1 flex-col items-center justify-center">
          <div className="w-full rounded-2xl bg-background p-4 text-center shadow-shadow-6 md:p-6">
            <span className="mcaption-regular16">
              {tContentQuiz('questionNum', {
                questionNum: current_question_index,
              })}
            </span>
            <p className="mcaption-semibold16 text-current">{question?.title}</p>
          </div>

          {question?.files?.length > 0 && (
            <div className="mt-6 w-full rounded-2xl bg-background p-3 text-center shadow-shadow-6">
              <Image
