...
const getContentClassName = (isOnlyContent: boolean, additionalClasses = '') =>
  cn(CONTENT_STYLES.common[isOnlyContent ? 'default' : 'multi'], additionalClasses);

/**
 * Wrapper Component for content types need max-height
 */
const ContentWrapper = ({ children, maxHeight, className = '', fullHeight = false }: IContentWrapperProps) => (
  <div
    style={{
      maxHeight,
...
export const CONTENT_RENDERERS: Record<TLessonContent, ContentRenderer> = {
  video: {
    render: ({ data, courseData, onCompleteContent, isOnlyContent, isPreview, lessonMetadataHeight = 0 }) => {
      const file = data?.files?.[0];
      const quizzes = data?.quizzes && data?.quizzes?.length > 0 ? data.quizzes : undefined;
    ...
    getClassName: isOnlyContent => getContentClassName(isOnlyContent, CONTENT_STYLES.video),
  },
  ...
  embedded: {
    render: ({ data, onCompleteContent, isOnlyContent, lessonMetadataHeight = 0 }) => {
      const maxHeight = useMemo(() => calculateMaxHeight(lessonMetadataHeight, 'embedded'), [lessonMetadataHeight]);
    ...
    getClassName: isOnlyContent => getContentClassName(isOnlyContent, CONTENT_STYLES.embedded),
  },

  quiz: {
    render: ({ data, courseData, onCompleteContent, isPreview, lessonMetadataHeight = 0 }) => {
    ...
    getClassName: isOnlyContent => getContentClassName(isOnlyContent, CONTENT_STYLES.quiz),
  },

  text: {
    render: ({ data, onCompleteContent }) => (
      <ContentText data={data} onComplete={() => onCompleteContent?.({ uid: data?.uid })} />
    ),
    getClassName: isOnlyContent => getContentClassName(isOnlyContent),
  },
};
