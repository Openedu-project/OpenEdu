...
import { type SocialProvider, getAPIReferrerAndOrigin } from "@oe/api";
...
const createHref = async (provider: SocialProvider) => {
  const { referrer } = await getAPIReferrerAndOrigin();
  const inviteRefCode = await getCookie(
    process.env.NEXT_PUBLIC_COOKIE_INVITE_REF_CODE
  );
  ...
  return (
    <>
      ...
      >
        ...
        <span className="giant-iheading-regular16">
          {tSocial("signInWithGoogleButton")}
        </span>
      </Button>
