...
export async function AuthConfirmPage({
  banner,
  themeName = "academia",
}: AuthConfirmProps) {
  ...
  if (event !== authEvents.setPassword && event !== authEvents.resetPassword) {
    try {
      ...
      if (response?.require_set_password) {
        try {
          decodedToken = base64ToJson(
            decodeURIComponent(response?.token ?? "")
          );
        } catch {
          decodedToken = null;
        }
          ...
          >
            <>
              <p className="text-muted-foreground text-sm">
                {tAuth("authConfirm.setPasswordDescription")}
              </p>
              <AuthConfirmForm
                event={event}
        ...
        return (
          ...
          >
            <div className="flex flex-col gap-y-4">
              <p className="text-muted-foreground text-sm">
                {tAuth("authConfirm.confirmDescriptionWithoutLogin")}
              </p>
              <div className="flex gap-x-2">
                <Link
                  href={`${AUTH_ROUTES.login}?next_path=${nextPath}`}
                  variant="default"
                  replace
                  className="w-full"
                >
        ...
        >
          <div className="flex flex-col gap-y-4">
            <p className="text-destructive text-sm">
              {tErrors((error as Error).message)}
            </p>
