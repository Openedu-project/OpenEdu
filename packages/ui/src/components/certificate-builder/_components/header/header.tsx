...
import {
  API_ENDPOINT,
  type ICertificateElement,
  type ICertificateRichTextElement,
  type ICertificateTextElement,
} from "@oe/api";
import { updateCertHtmlTemplateService } from "@oe/api";
...
export const Header = () => {
  ...
  const handleSave = async () => {
    ...
    const enableProject = template?.elements?.some(
      (element: ICertificateElement) =>
        (
          element as ICertificateRichTextElement | ICertificateTextElement
    ...
    try {
      ...
      await globalMutate(
        (key: string) => !!key?.includes(`${API_ENDPOINT.HTML_TEMPLATES}?`),
        undefined,
        {
          revalidate: false,
        }
    ...
    <div className="flex h-14 items-center gap-4 border-b bg-background px-4">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          {isSaving
            ? tCertificate("builder.saving")
            : tCertificate("builder.save")}
        </Button>
      </div>
    </div>
  );
