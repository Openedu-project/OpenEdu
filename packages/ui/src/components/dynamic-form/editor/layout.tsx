      ...
      >
        <div className="flex flex-wrap justify-between gap-2">
          <div className="flex flex-1 flex-col gap-2 md:flex-row md:items-center">
            <Input name="name" placeholder="Form name" className="min-w-60 max-w-sm" autoComplete="off" />
            <Input name="description" placeholder="Form description" className="min-w-60 max-w-sm md:max-w-full" />
          </div>
          <Button variant="default">{tGeneral('save')}</Button>
        </div>
      </DashboardHeaderCard>
      <div className="flex h-full flex-1 flex-col overflow-hidden md:flex-row">
        <div className="scrollbar flex flex-shrink-0 flex-wrap gap-2 overflow-auto bg-background p-2 md:block md:w-48">
          <Templates />
          <Components />
        </div>
