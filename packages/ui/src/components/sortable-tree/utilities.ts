...
export function buildTree(flattenedItems: FlattenedItem[]) {
  const root: TreeItem = { id: 'root', title: '', children: [] };
...
export function updateTree(items: TreeItems, id: UniqueIdentifier, updates: Partial<TreeItem>): TreeItems {
  return items.map(item => {
    if (item.id === id) {
      return { ...item, ...updates };
    }
    if (item.children) {
      return { ...item, children: updateTree(item.children, id, updates) };
    }
    return item;
  });
}
...
export function setProperty<T extends keyof TreeItem>(
  ...
  return JSON.parse(JSON.stringify(items)) as TreeItems;
}
