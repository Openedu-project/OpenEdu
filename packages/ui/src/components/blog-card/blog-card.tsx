...
export function BlogCardServer({
  ...
  const authorFullUrl = isExternalDomain ? `https://${blog?.org?.domain}/${locale}${authorUrl}` : authorUrl;

  return (
    <Link href={blogUrl} target={isExternalDomain ? '_blank' : undefined}>
      <Card
        ...
        <CardContent className={cn('!p-3 flex basis-full flex-col gap-3', contentRight && 'grid-cols-2 md:grid')}>
          {authorOnTop && (
            <div className={cn('mb-2 flex items-center justify-between gap-1', contentRight && 'md:col-span-2')}>
              <Link href={authorFullUrl} target={isExternalDomain ? '_blank' : undefined}>
                <ProfileCard
