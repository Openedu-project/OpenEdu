        ...
        >
          <div className="flex flex-grow flex-wrap items-center gap-1 p-2">
            {value
              ...
              .slice(0, isExpanded ? value.length : (displayItems ?? value?.length ?? 0))
              .map(option => (
                <OptionBadge
                  key={getOptionValue(option)}
            ...
            {displayItems && value?.length > 0 && !isExpanded && displayItems < value?.length && (
              <Badge className="cursor-pointer" onClick={toggleExpand} variant="secondary">
                <Plus className="mr-1 h-2 w-2" />
                {value.length - displayItems}
              </Badge>
            )}
            {displayItems && value?.length > 0 && isExpanded && displayItems < value?.length && (
              <Badge className="cursor-pointer" variant="secondary" onClick={toggleExpand}>
