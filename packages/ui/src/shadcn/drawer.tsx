...
const DrawerHeader = ({ className, ...props }: HTMLAttributes<HTMLDivElement>) => (
  <div data-slot="header" className={cn('grid gap-1.5 p-4 text-center sm:text-left', className)} {...props} />
);
DrawerHeader.displayName = 'DrawerHeader';

const DrawerFooter = ({ className, ...props }: HTMLAttributes<HTMLDivElement>) => (
  <div data-slot="footer" className={cn('mt-auto flex flex-col gap-2 p-4', className)} {...props} />
);
